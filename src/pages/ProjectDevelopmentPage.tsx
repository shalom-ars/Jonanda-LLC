    ...
    <>
      ...
      <div className="pt-32 pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-28 relative z-10">
        ...
        <section className="text-center space-y-8 max-w-4xl mx-auto relative">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-gold-500/10 border border-gold-500/30 text-xs font-semibold text-gold-300 shadow-gold-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {WHAT_WE_BUILD_SERVICES.map((service) => {
              ...
              return (
                <CorporateCard
                  key={service.id}
                  className="p-8 flex flex-col justify-between h-full border-white/[0.08] hover:border-gold-500/30 transition-all duration-300"
                >
                  <div className="space-y-6">
                    <div className="flex items-center justify-between">
                      <div className="w-12 h-12 rounded-xl bg-gold-500/10 border border-gold-500/30 text-gold-400 flex items-center justify-center shadow-gold-sm">
                        <IconComp className="w-6 h-6" />
                      </div>
                      <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-white/[0.04] border border-white/[0.08] text-gray-400">
                        {service.category}
                      </span>
                    </div>
                  ...
                  <div className="pt-6 mt-6 border-t border-white/[0.06]">
                    <a
                      href="#project-inquiry"
                      className="text-xs font-semibold text-gold-300 hover:text-gold-200 inline-flex items-center gap-1.5 transition-colors"
                    >
        ...
        <section className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {DEVELOPMENT_PROCESS_STEPS.map((step) => (
              <CorporateCard
                key={step.stepNumber}
                className="p-6 flex flex-col justify-between h-full border-white/[0.08] hover:border-gold-500/30 transition-all duration-300 relative overflow-hidden"
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-3xl font-extrabold font-mono text-gold-400/80">
                      {step.stepNumber}
                    </span>
                    <span className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 px-2 py-0.5 rounded bg-white/[0.04] border border-white/[0.08]">
                ...
                <div className="pt-4 mt-4 border-t border-white/[0.06] space-y-1.5">
                  ...
                  {step.deliverables.map((item, dIdx) => (
                    <div key={dIdx} className="text-[11px] text-gray-300 flex items-center gap-1.5">
        ...
        <section className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {TECHNOLOGY_CATEGORIES.map((cat, idx) => (
              <CorporateCard key={idx} className="p-7 space-y-5 border-white/[0.08]">
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {cat.technologies.map((tech, tIdx) => (
                    <span
                      key={tIdx}
                      className="px-3 py-1.5 rounded-lg bg-background/80 border border-white/[0.08] text-xs font-mono text-gray-200 hover:border-gold-500/40 hover:text-gold-200 transition-colors"
                    >
                      {tech}
                    </span>
                  ))}
            ...
            {PROJECT_TYPES_LIST.map((prj, pIdx) => (
              ...
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-gold-500/10 text-gold-300 border border-gold-500/20">
                      {prj.tag}
                    </span>
                  </div>
                ...
                <div className="pt-4 mt-4 border-t border-white/[0.06]">
                  <a
                    href="#project-inquiry"
                    className="text-[11px] font-semibold text-gray-400 hover:text-white inline-flex items-center gap-1 transition-colors"
                  >
        ...
        <section className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {WHY_BUILD_WITH_JONANDA.map((item, idx) => (
              <CorporateCard key={idx} className="p-8 space-y-4 border-white/[0.08] hover:border-gold-500/30 transition-all duration-300">
                <span className="text-xs font-bold uppercase tracking-wider text-gold-400 block font-mono">
                  {item.highlight}
                </span>
                <h3 className="text-xl font-bold text-white">
                  {item.title}
        ...
        {/* ========================================================= */}
        <section className="rounded-3xl bg-gradient-to-b from-surface/80 to-surface/40 border border-white/[0.08] p-8 sm:p-12 space-y-6 relative overflow-hidden backdrop-blur-md">
