...
import { ExternalLink, ArrowRight, Sparkles, Shield, Bot, Search, Users, ShieldAlert, Cpu, Layers, CheckCircle2, Clock } from 'lucide-react';
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="shrink-0">
            {product.status === 'Live & Operational' && (
              <span className="inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider bg-emerald-500/15 text-emerald-300 border border-emerald-500/30">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
                Live & Operational
              </span>
            )}
            {product.status === 'Active Platform' && (
              <span className="inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider bg-amber-500/15 text-amber-300 border border-amber-500/30">
                <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-pulse" />
                Active Platform
              </span>
            )}
            {product.status === 'Coming Soon' && (
              <span className="inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider bg-purple-500/15 text-purple-200 border border-purple-500/30 shadow-sm">
                <Clock className="w-3 h-3 text-purple-300" />
                COMING SOON
              </span>
            )}
            {product.status === 'Incubation & Research' && (
              <span className="inline-flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider bg-blue-500/15 text-blue-300 border border-blue-500/30">
      ...
      <div className="pt-6 mt-6 border-t border-white/[0.06]">
        {isComingSoon ? (
          <div className="w-full py-2.5 px-4 rounded-lg bg-surface/80 border border-purple-500/20 text-purple-200 text-xs font-semibold flex items-center justify-between cursor-default select-none">
